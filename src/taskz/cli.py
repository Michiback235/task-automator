...
from taskz.config import load_config, save_config, config_path
from taskz.db.connection import connect, apply_migrations
from taskz.logging import setup_logging
from taskz.utils.paths import iter_files, safe_rel
from taskz.files.renamer import preview as preview_renames, execute as execute_renames
...
@click.option("--src", type=click.Path(path_type=Path, exists=True, file_okay=False), required=True)
@click.option("--dest", type=click.Path(path_type=Path, file_okay=False), default=None, help="Defaults to --src")
...
def files_rename(src: Path, dest: Path | None, recursive: bool, pattern: str, lowercase: bool, dedupe: bool, yes: bool):
...
@click.option("--from", "from_path", type=click.Path(path_type=Path, exists=True, file_okay=False), required=True)
@click.option("--globs", type=str, default="*.html,*.json", help="Comma-separated glob patterns")
@click.option("--vendor", type=click.Choice(["auto", "generic_html", "jumia", "uber"]), default="auto")
...
def rule_list():
    for r in list_rules():
        click.echo(f"[{r['id']}] {r['enabled']} prio={r['priority']} if ~{r['pattern']} => set {r['target_field']}={r['value']}")
