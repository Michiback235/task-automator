...
from taskz.receipts.parsers import detector, generic_html, uber, jumia
...
def _insert_receipt(conn: sqlite3.Connection, src_type: str, src_path: str, raw: str, vendor: str, normalized: NormalizedReceipt) -> int:
    h = content_hash(raw)
    ...
    with conn:
        cur = conn.execute(
            """
            INSERT INTO receipt(source_type, source_path_or_url, content_hash, raw_text, parsed_ok, parsed_at, vendor, meta_json)
        ...
        conn.execute(
            """
            INSERT INTO expense(date, merchant, category, description, amount_net, tax, amount_gross, currency, payment_method, tags_csv, source, source_id, receipt_id)
            VALUES (date('now'), ?, NULL, ?, NULL, NULL, ?, ?, NULL, NULL, 'receipt', ?, ?)
            """,
            (normalized.merchant, f"Imported from receipt {vendor}", normalized.total_gross, normalized.currency, h[:12], rid),
        )
    return rid
